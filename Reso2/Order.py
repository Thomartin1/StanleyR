...
# Noter que movespeed et get speed sont plutot des temps pour parcourir une certaine distance. Plus ils sont grand , moins grande est la vitesse
    ...
    # return time
    return (datetime.timedelta(0,180))
...
class Task:
    ...
    def update(self,start):
        ...
        self.delay=max(self.effectivestart-self.startfrom,self.startfrom-self.startfrom)
    ...
    def printincsv(self,robnum,cust_arrival,cust_departure,Nbrobonduty):
        ...
        inforow=[str(self.startfrom),str(self.type),str(robnum),str(self.identity),str(cust_arrival),str(self.effectivestart),str(self.effectiveend),str(cust_departure),str(self.delay),str("nbdeposit"),str("Nbstorage"),str("Nbrobonduty"),str(self.begin),str(self.end)]
        wr.writerow(inforow)
        log.close()
